package com.blackswan.fake.Interceptor;
...
public class LoginedCheckInterceptor extends AbstractInterceptor {
    private static final long serialVersionUID = 1719746357534136007L;
	...
	public String intercept(ActionInvocation ai) throws Exception {
		...
		if(!ServletActionContext.getRequest().isRequestedSessionIdValid()){
			...
			if (url.indexOf("admin_login.action")!=-1 || url.indexOf("admin_logout.action")!=-1){
				...
				admin = (Administrator)ServletActionContext.getRequest().getSession().getAttribute("admin");
				//验证是否已经登录
				if (admin==null){
					//尚未登录,跳转至登录页面
					return "tologin";
					...
					//取得当前用户的操作权限
					prim = admin.getAdprivileges().trim();
					//进行权限验证
					if (index>0){
